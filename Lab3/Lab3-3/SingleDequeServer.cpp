...
int max_windows_size=32;
int now_windows_size;
...
int RecvMessage(SOCKET& sockServ, SOCKADDR_IN& ClientAddr, int& ClientAddrLen, char *message,deque<int>& d)
{
    ...
    while (1 == 1)
    {
        ...
        {
            //cout<<"flag=0;"<<endl;
            now_windows_size=int(header.windows_size);
            seq = int(header.SEQ);
            if (seq > 255)
            {
                seq = seq - 256;
            ...
            if(seq == windowspos)
            {
                 ...
                 //取出buffer中的内容
                std::cout << "Receive message " << length - sizeof(header) << " bytes! "<<" SEQ:" << int(header.SEQ) 
                            << " SUM:" << int(header.sum)<<" 现窗口大小:"<< now_windows_size << endl;
            ...
            // 这里逻辑判断还是有点弱，应该判断一下之前是否接受过这样的包。但是实测下来不会重发。
                ...
                //recvNum++;
                resetColor();
                std::cout << "Receive message. SEQ:" << int(header.SEQ) 
                            << " SUM:" << int(header.sum)<<" 现窗口大小:"<< now_windows_size << endl;
                //将接收到的数据包存起来。

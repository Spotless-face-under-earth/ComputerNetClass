...
const int MAX_windows_size = 32;//最大窗口大小
const int INI_windows_size = 1;//初始窗口大小
int now_windows_size;//当前窗口大小
...
void send_package(SOCKET& socketClient, SOCKADDR_IN& servAddr, int& servAddrlen, char* message, int len, int order,int windows_size)
{
    u_long mode=1;
    ...
    header.datasize = len;
    header.windows_size=windows_size;
...
void sender(SOCKET& socketClient, SOCKADDR_IN& servAddr, int& servAddrlen, char* message, long long int len,int& resendCount, int& base ,deque<int>& d, mutex& mtx){
    ...
    seqnum=base;
    clock_t start=clock();
    now_windows_size=INI_windows_size;
    int i = 0;
    //int initk=1;
    ...
    {        
        {
            ...
            if(i<=5 && now_windows_size < MAX_windows_size){
                setColor(35);
                cout<<"慢启动阶段,当前窗口大小:"<<now_windows_size<<endl;
                resetColor();
                now_windows_size*=2;
            }
           
            else if(now_windows_size < MAX_windows_size){
                now_windows_size*=2;
                setColor(35);
                cout<<"丢包恢复中,当前窗口大小:"<<now_windows_size<<endl;
                resetColor();
        ...
        //std::unique_lock<std::mutex> lock(mtx);
        if((seqnum>=base && seqnum<base+now_windows_size)||
            (seqnum<base && seqnum+256-base<now_windows_size))
        {
            
            {
                ...
                setColor(33);
                // 输出文本行
                //std::cout << "Send windows: ["<<base<<"……"<<base+now_windows_size<<"] " 
                ...
                send_package(socketClient, servAddr, servAddrlen, message + i * MAXSIZE, i == packagenum - 1? len - (packagenum - 1) * MAXSIZE : MAXSIZE, seqnum,now_windows_size);
                start=clock();
            }
            ...
            if(clock()-start >  MAX_TIME){
                ...
                if(resendCount<=3){
                    ...
                    send_package(socketClient, servAddr, servAddrlen, message + d.front() * MAXSIZE, i == packagenum - 1? len - (packagenum - 1) * MAXSIZE : MAXSIZE, prenum,now_windows_size );
                    ...
                    resetColor();
                    cout<<"Sleep. "<<endl;
                    cv.wait(lock, [&]() { return ((seqnum >= base && seqnum < base + now_windows_size) || 
                                         (base + now_windows_size> 255 && seqnum < (base + now_windows_size) % 256)); 
                                          });
                    start=clock();
                    i--;
                }
    ...
    if(!d.empty()){      
        for (deque<int>::iterator it = d.begin(); it != d.end(); ++it) {
            send_package(socketClient, servAddr, servAddrlen, message + (*it * MAXSIZE), i == packagenum - 1? len - (packagenum - 1) * MAXSIZE : MAXSIZE, *it,now_windows_size);
...
void sendName(SOCKET& socketClient, SOCKADDR_IN& servAddr, int& servAddrlen, char* message, long long int len,int& resendCount, int& base ,mutex& mtx){
    ...
    {        
            ...
            (seqnum<base&&seqnum+256-base<now_windows_size))
        {
            ...
            send_package(socketClient, servAddr, servAddrlen, message, i == packagenum - 1? len - (packagenum - 1) * MAXSIZE : MAXSIZE, seqnum,1);
            start=clock();
        }
        else{
...
void receiver(SOCKET& socketClient, SOCKADDR_IN& servAddr, int& servAddrlen,int&resendCount, int& base,deque<int>& d, mutex& mtx) {
    ...
    while (true) {
        ...
        if (receivedBytes > 0) {
            ...
            if (((recvheader.SEQ >= base && recvheader.SEQ < base + MAX_windows_size)||
                (recvheader.SEQ<base && recvheader.SEQ+MAX_SEQ_NUM-base< MAX_windows_size))&& recvheader.flag==ACK) {
                ...
                if(base == recvheader.SEQ){
                    ...
                    if(spiltPacket){
                        ...
                        now_windows_size=now_windows_size/4;
                        cout<<"文件传输过程丢包，当前窗口大小:"<<now_windows_size<<endl;
                        spiltPacket=false;
                    }
                ...
                cout << "Recv wrong packet:" << int(recvheader.flag)<<" Base:"<<base<< endl;
                system("pause");
                resetColor();
                cv.notify_all();

...
const int MAX_windows_size =64;//最大窗口大小
...
void sender(SOCKET& socketClient, SOCKADDR_IN& servAddr, int& servAddrlen, char* message, long long int len,int& resendCount, int& base ,deque<int>& d, mutex& mtx){
    ...
    {        
            ...
            (seqnum<base && seqnum+256-base<MAX_windows_size))
        {
            
            {
                ...
                send_package(socketClient, servAddr, servAddrlen, message + i * MAXSIZE, i == packagenum - 1? len - (packagenum - 1) * MAXSIZE : MAXSIZE, seqnum,MAX_windows_size);
                start=clock();
            }
            ...
            if(clock()-start >  MAX_TIME){
                ...
                if(resendCount<=3){
                    ...
                    send_package(socketClient, servAddr, servAddrlen, message + d.front() * MAXSIZE, i == packagenum - 1? len - (packagenum - 1) * MAXSIZE : MAXSIZE, prenum, MAX_windows_size );
                    ...
                    resetColor();
                    cout<<"Sleep. "<<endl;
                    cv.wait(lock, [&]() { return ((seqnum >= base && seqnum < base + MAX_windows_size) || 
                                         (base + MAX_windows_size> 255 && seqnum < (base + MAX_windows_size) % 256)); 
                                          });
                    start=clock();
                    i--;
                }
    ...
    if(!d.empty()){      
        for (deque<int>::iterator it = d.begin(); it != d.end(); ++it) {
            send_package(socketClient, servAddr, servAddrlen, message + (*it * MAXSIZE), i == packagenum - 1? len - (packagenum - 1) * MAXSIZE : MAXSIZE, *it,MAX_windows_size);
                ...
                if(base == recvheader.SEQ){
                    ...
                    if(d.empty()){
                        if(noSpilt){
                            ...
                            base=(base+MAX_windows_size>255?0:base+MAX_windows_size);
                            noSpilt=true;
                        }
                    }
                    else{
...
int main() {
    ...
    clock_t end=clock();
    setColor(31);
    cout<<"滑动窗口大小："<<MAX_windows_size<<endl;
    cout<<"文件发送时间："<<(double)(clock()-SendTime)/CLOCKS_PER_SEC<<"s. ";
    cout << "吞吐率为:" << ((float)index ) / ((end - SendTime) /CLOCKS_PER_SEC) << "byte/s" << endl;
    resetColor();

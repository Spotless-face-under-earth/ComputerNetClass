    ...
    while (1 == 1)
    {
        ...
        if (header.flag == 0 && cksum((u_short*)Buffer, length - sizeof(header)))
        {
            ...
            if (seq != int(header.SEQ))
            {
                ...
                setColor(33);
                cout <<"Needed SEQ:"<<seq<< " Resend to Clinet ACK:" << (int)header.SEQ << " SEQ:" << (int)header.SEQ << endl;
                resetColor();
                continue;//丢弃该数据包
            }
            seq = int(header.SEQ);
            ...
            //取出buffer中的内容
            cout << "Receive message " << length - sizeof(header) << " bytes! "<<"Flag:" << int(header.flag) << " SEQ : " << int(header.SEQ) << " SUM:" << int(header.sum) << endl;
